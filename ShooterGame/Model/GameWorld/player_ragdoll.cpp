...
PlayerRagdoll::PlayerRagdoll(GameWorld *world, const Vec3f &pos, const Quaternionf &orientation) : GameObject(world)
{
	...
	//SceneModel model(world->client->gc(), world->client->scene, "Thalania/Thalania.cmodel");
	...
	scene_object1 = SceneObject::create(world->client->scene, model);
	scene_object1->set_position(pos);
	scene_object1->set_orientation(orientation);

	scene_object2 = SceneObject::create(world->client->scene, model);
	scene_object2->set_position(pos);
	scene_object2->set_orientation(orientation);

	auto shape = Physics3DShape::box(box_size);
	physics_object1 = Physics3DObject::rigid_body(world->collision, shape, 1.0f, pos, orientation);
	physics_object1->set_ccd_swept_sphere_radius(1.0f);
	physics_object1->set_ccd_motion_threshold(0.5f);

	physics_object2 = Physics3DObject::rigid_body(world->collision, shape, 1.0f, pos + orientation.rotate_vector(Vec3f(0.0f, 0.0f, 1.0f)), Quaternionf::inverse(orientation));
	physics_object2->set_ccd_swept_sphere_radius(1.0f);
	physics_object2->set_ccd_motion_threshold(0.5f);

	hinge_constraint = Physics3DConstraint::hinge(world->collision, physics_object1, physics_object2, Vec3f(0.0f, 0.0f, 1.0f), Vec3f(0.0f, 0.0f, 1.0f), Quaternionf(90.0f, 0.0f, 0.0f, angle_degrees, order_YXZ), Quaternionf(90.0f, 0.0f, 0.0f, angle_degrees, order_YXZ));
	...
	for (int i = 0; i < total_parts; i++)
		parts[i] = Physics3DObject::rigid_body(world->collision, shapes[i], 1.0f);
	...
	joints[joint_head] = Physics3DConstraint::cone_twist(world->collision, parts[part_torso], parts[part_head], pos_a, pos_b, rotate_a, rotate_b);
	joints[joint_head].set_cone_twist_limit(0.7f, 0.5f, 0.0f);

	joints[joint_upper_arm_right_lower_arm_right] = Physics3DConstraint::hinge(world->collision, parts[part_upper_arm_right], parts[part_lower_arm_right], pos_a, pos_b, rotate_a, rotate_b);
	joints[joint_upper_arm_right_lower_arm_right].set_hinge_limit(-PI_F / 2.0f, 0);

	joints[joint_upper_arm_left_lower_arm_left] = Physics3DConstraint::hinge(world->collision, parts[part_upper_arm_left], parts[part_lower_arm_left], pos_a, pos_b, rotate_a, rotate_b);
	joints[joint_upper_arm_left_lower_arm_left].set_hinge_limit(-PI_F / 2.0f, 0);

	joints[joint_upper_leg_right_lower_leg_right] = Physics3DConstraint::hinge(world->collision, parts[part_upper_leg_right], parts[part_lower_leg_right], pos_a, pos_b, rotate_a, rotate_b);
	joints[joint_upper_leg_right_lower_leg_right].set_hinge_limit(-PI_F / 2.0f, 0);

	joints[joint_upper_leg_left_lower_leg_left] = Physics3DConstraint::hinge(world->collision, parts[part_upper_leg_left], parts[part_lower_leg_left], pos_a, pos_b, rotate_a, rotate_b);
	joints[joint_upper_leg_left_lower_leg_left].set_hinge_limit(-PI_F / 2.0f, 0);

	joints[joint_torso_upper_leg_right] = Physics3DConstraint::cone_twist(world->collision, parts[part_torso], parts[part_upper_leg_right], pos_a, pos_b, rotate_a, rotate_b);
	joints[joint_torso_upper_leg_right].set_cone_twist_limit(PI_F / 4.0f, PI_F / 4.0f);

	joints[joint_torso_upper_leg_left] = Physics3DConstraint::cone_twist(world->collision, parts[part_torso], parts[part_upper_leg_left], pos_a, pos_b, rotate_a, rotate_b);
	joints[joint_torso_upper_leg_left].set_cone_twist_limit(PI_F / 4.0f, PI_F / 4.0f);

	joints[joint_torso_upper_arm_right] = Physics3DConstraint::cone_twist(world->collision, parts[part_torso], parts[part_upper_arm_right], pos_a, pos_b, rotate_a, rotate_b);
	joints[joint_torso_upper_arm_right].set_cone_twist_limit(PI_F / 2.0f, PI_F / 2.0f);

	joints[joint_torso_upper_arm_left] = Physics3DConstraint::cone_twist(world->collision, parts[part_torso], parts[part_upper_arm_left], pos_a, pos_b, rotate_a, rotate_b);
