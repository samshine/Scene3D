...
TestSceneCache::TestSceneCache()
{
}

std::shared_ptr<ModelData> TestSceneCache::get_model_data(const std::string &name)
{
	if (name == "plane")
		return create_plane();
	else if (name == "box")
...
Resource<Texture> TestSceneCache::get_texture(GraphicContext &gc, const std::string &name, bool linear)
{
	...
	else
	{
		...
		try
		{
			PixelBuffer pb(name, !linear);

			Texture2D tex(gc, pb.get_width(), pb.get_height(), tf_rgba8, (int)std::max(log2l(pb.get_width()), log2l(pb.get_height())));
			tex.set_image(gc, pb);
			tex.generate_mipmap();
		...
		catch (Exception &)
		{
			...
			PixelBuffer pb(1, 1, tf_rgba8, &pixel);
			texture.set(Texture2D(gc, pb));
		}
...
void TestSceneCache::update_textures(GraphicContext &gc, float time_elapsed)
{
	// Update animated textures here.
}

std::shared_ptr<ModelData> TestSceneCache::create_plane()
{
...
std::shared_ptr<ModelData> TestSceneCache::create_box()
{
	std::shared_ptr<ModelData> model_data(new ModelData());
