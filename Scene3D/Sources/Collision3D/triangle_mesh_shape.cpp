...
namespace uicore
{
	...
	bool TriangleMeshShape::find_any_hit(TriangleMeshShape *shape, const Vec3f &ray_start, const Vec3f &ray_end)
	{
	...
	bool TriangleMeshShape::find_any_hit(TriangleMeshShape *shape, const Vec3f &ray_start, const Vec3f &ray_end, int a)
	{
	...
	bool TriangleMeshShape::overlap_bv_ray(TriangleMeshShape *shape, const Vec3f &ray_start, const Vec3f &ray_end, int a)
	{
		return IntersectionTest::ray_aabb(ray_start, ray_end, shape->nodes[a].aabb) == IntersectionTest::overlap;
	}

	float TriangleMeshShape::intersect_triangle_ray(TriangleMeshShape *shape, const Vec3f &ray_start, const Vec3f &ray_end, int a)
	{
		...
		Vec3f P = Vec3f::cross(D, e2);
		float det = Vec3f::dot(e1, P);
		...
		Vec3f Q = Vec3f::cross(T, e1);

		// Calculate V parameter and test bound
		float v = Vec3f::dot(D, Q) * inv_det;

		// The intersection lies outside of the triangle
		if (v < 0.f || u + v  > 1.f)
			return 1.0f;

		float t = Vec3f::dot(e2, Q) * inv_det;
		if (t > FLT_EPSILON)
			return t;
