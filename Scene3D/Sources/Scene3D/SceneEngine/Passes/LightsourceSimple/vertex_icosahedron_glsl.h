namespace { const char *vertex_icosahedron_glsl() { return R"shaderend(
...
void main()
{
	...
	vec3 VertexInEye = PixelPositionInEye.xyz + AttrPositionInObject.xyz * sqrt(PixelRange.x);

	gl_Position = EyeToProjection * vec4(VertexInEye, 1);
}
