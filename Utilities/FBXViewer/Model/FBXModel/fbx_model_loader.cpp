...
FBXModelLoader::FBXModelLoader(FBXModel *model, const std::vector<Animation> &animations, const std::vector<AttachmentPoint> &attachment_points, const std::vector<ParticleEmitter> &emitters)
	: model(model), model_data(std::make_shared<ModelData>())
{
	convert_node(model->scene->GetRootNode());
	for (const auto &anim : animations) convert_bones(anim);
}
...
void FBXModelLoader::convert_bones(const Animation &animation_desc)
{
