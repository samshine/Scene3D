...
std::shared_ptr<ModelData> FBXModel::convert(const std::vector<Animation> &animations, const std::vector<AttachmentPoint> &attachment_points, const std::vector<ParticleEmitter> &emitters)
{
	FBXModelLoader loader(this, animations, attachment_points, emitters);
	return loader.model_data;
}

void FBXModel::triangulate_scene()
{
	FbxGeometryConverter converter(manager);
	converter.Triangulate(scene, true);
}

void FBXModel::import_scene(const std::string &filename)
{
...
void FBXModel::bake_geometric_transforms(FbxNode *node)
{
